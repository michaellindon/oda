...
List mixture_var(NumericVector ryo, NumericMatrix rxo, NumericMatrix rxa, NumericVector rlam, NumericVector rpriorprob, SEXP ralpha){
  ...
  phi=1;
	...
	do{
		...
		b=0.5*dot(yoc-xo*P*mu_B,yoc-xo*P*mu_B)+0.5*dot(mu_B,Lam*P*mu_B)+0.5*dot(mu_B,(P-P*P)*D*mu_B)+0.5*sum(prob%E_B%(lam+d));
		a=0.5*(no+sum(prob)-1);
		phi=((double)a)/b;
		...
		for (int i = 0; i < p; ++i)
		{
			lam(i)=(alpha+prob(i))/(alpha+phi*prob(i)*(mu_B(i)+dli(i)));
		}
		Lam=diagmat(lam);
		ldl=sqrt(lam/(d+lam));
		dli=1/(d+lam);
